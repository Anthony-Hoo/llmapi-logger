...
6. 仅审计路由把 Accept-Encoding 固定为 `identity`。采集读的是上游原始字节，content-encoded 的响应会让 SSE 分帧失去意义、流终止事件不可见，而后者正是区分「终止事件后挂断」与真实取消的依据。Transport 自身不发 Accept-Encoding（DisableCompression=true），只有客户端带的头会重新引入编码。客户端按 RFC 9110 用 `identity;q=0`，或用 `*;q=0` 且未另行允许 identity 时，属于明确拒绝 identity，此时保留其原始头不覆盖——不能为了便于采集就向 NewAPI 索取客户端已经拒绝的表示；该请求的流因此无法识别终止事件，退回原取消分类。入站原值在任何情况下都逐字记录在 request_for_newapi_received_from_nginx 阶段。passthrough 不审计，照常协商。
