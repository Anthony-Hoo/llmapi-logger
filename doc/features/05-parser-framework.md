...
`internal/parser` 保存公共接口、内存队列和 JSON/SSE 解码；`openai`、`anthropic`、`gemini` 子包保存协议实现。`proxy`、`audit` 和 `capture` 不依赖 parser，parser 只依赖只读证据、`parsed_results` 写入和加密接口。
...
type Result struct {
    ...
    ToolCallCount   int
    HasToolCall     bool
    ParsedJSON      []byte // 加密后落盘
}
```

公共查询字段只包含模型、流式标志、response ID、usage、错误类型/代码、消息数量和工具调用数量。消息正文、输出文本、工具/函数名称与参数、完整错误信息只进入加密 `parsed_json`。
...
reparse 只需把目标 audit 的 `parse_status` 设为 `pending` 并重新入队；解析成功后覆盖该 audit 的最新 `parsed_results`。
...
parser 主要读取第一阶段的请求 Body 和第三阶段的响应 Body，并使用另外两个阶段判断实际转发状态与完整性。阶段行按实际触发存在，不能把缺失行当成空 Body；rejected audit 不进入 parser。
...
- JSON 使用流式 decoder 和 `UseNumber`，未知字段忽略或保存在加密 `parsed_json`。
- SSE reader 支持 LF/CRLF、跨 chunk 行、多条 `data:` 和正常 EOF。
- 不为解析完整 `io.ReadAll` 超大 Body，也不重新编码原始证据。
...
- reparse 覆盖最新 `parsed_results`，不修改原始证据。
- rejected audit 保持 skipped，不进入启动扫描、周期扫描或 reparse worker，且不创建 `parsed_results`。
...
4. 接入三个协议 parser、加密 `parsed_json` 和覆盖式 reparse。
