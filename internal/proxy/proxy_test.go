...
// stubSink is an available-mode sink that admits every request without
// creating a session, which is enough to select the audited proxy behaviour.
type stubSink struct{}

func (stubSink) Healthy() bool { return true }
func (stubSink) Mode() string  { return "available" }
func (stubSink) Begin(context.Context, *http.Request, routing.Match) (*audit.Session, error) {
	return nil, nil
}

// Audited routes must reach NewAPI with identity encoding: the capture path
// reads raw upstream bytes, so a content-encoded response would leave SSE
// framing meaningless and the stream terminal marker invisible. Passthrough is
// not audited and keeps negotiating whatever the client asked for.
func TestAcceptEncodingNormalizedOnlyForAuditedRoutes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name:    "passthrough",
			handler: NewPassthroughWithOptions(target, Options{}, nil),
		...
		t.Run(test.name, func(t *testing.T) {
			...
			test.handler.ServeHTTP(httptest.NewRecorder(), request)
			select {
			case got := <-observed:
				if got != test.want {
