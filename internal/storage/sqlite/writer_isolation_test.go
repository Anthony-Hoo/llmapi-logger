...
func TestWriterIsolatesStoredTurnGraphFailureFromQueuedCapture(t *testing.T) {
	...
	for _, test := range []struct {
		name, tamper, message string
	}{
		{"sequence hash", "UPDATE turns SET request_sequence_hash = zeroblob(32) WHERE turn_id = 'parent-turn'", "sequence hash mismatch"},
		{"delta reconstruction", "INSERT INTO turn_context_ops (turn_id, op_index, operation, item_count) VALUES ('parent-turn', 99, 'retain', 5)", "reconstruction failed"},
		{"root base", "UPDATE turns SET parent_base = 'request' WHERE turn_id = 'parent-turn'", "root turn has non-root base"},
		{"parent cycle", "UPDATE turns SET parent_turn_id = 'parent-turn', parent_base = 'request' WHERE turn_id = 'parent-turn'", "turn parent cycle"},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := store.writerDB.Exec(test.tamper); err != nil {
				t.Fatal(err)
			}
			...
			if err := <-batch[2].ack; err == nil || !canIsolateWriteError(err) || !strings.Contains(err.Error(), test.message) {
				t.Fatalf("stored turn graph failure = %v, want an isolated %q failure", err, test.message)
			}
			if !writer.healthy.Load() {
